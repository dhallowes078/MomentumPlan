    ...
    providers: [
            ...
            Credentials({
              ...
              async authorize() {
                try {
                  ...
                  await ensurePersonalWorkspace(user.id, user.name ?? "Local User", client);
                  await ensureAccessCode(user.id, client);
                  return {
                    id: user.id,
                    name: user.name,
                    email: user.email,
      ...
      Credentials({
        ...
        async authorize(credentials) {
          ...
          const user = await client.user.findUnique({ where: { accessCode: code } });
          if (!user) return null;
          return {
            id: user.id,
            name: user.name,
    ...
    events: {
      async createUser({ user }) {
        if (user.id && user.email) {
          const client = await getPrisma();
          await ensurePersonalWorkspace(user.id, user.name ?? user.email, client);
          await ensureAccessCode(user.id, client).catch(console.error);
        }
      },
      async linkAccount({ user, account }) {
        if (
          ...
          const client = await getPrisma();
          await client.calendarConnection.upsert({
          ...
          await client.schedulePrefs.upsert({
            where: { userId: user.id },
      ...
      async signIn({ user, account }) {
        ...
        if (user.id && account?.provider === "local") {
          await ensurePersonalWorkspace(user.id, user.name ?? user.email ?? "Local User", client);
          await ensureAccessCode(user.id, client).catch(console.error);
        }

        if (user.id && (account?.provider === "device-code" || account?.provider === "google")) {
          await ensurePersonalWorkspace(user.id, user.name ?? user.email ?? "User", client);
          await ensureAccessCode(user.id, client).catch(console.error);
        }
